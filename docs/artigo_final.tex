...
O algoritmo detectou automaticamente 3 comunidades distintas. A inspeção visual (Figura \ref{fig:clusters}) e analítica revela que estes grupos correspondem quase perfeitamente às grandes alianças gloais (ex: VW Group, Stellantis).
...
Investigamos se existe segregação tecnológica entre carros de luxo (Premium) e carros populares (Economy). O coeficiente de assortatividade calculado foi $r = -0.008$.
    O valor próximo de zero (ou negativo) é surpreendente e revela uma 	extbf{homogeneização tecnológica}. Componentes críticos (freios, sensores, eletrônica) tornou-se commodities genéricas, utilizadas indistintamente por marcas Premium e generalistas. Um Audi A3 e um VW Golf compartilham a mesma 'alma' mecânica.
